class_name DashDraugrStateIdle extends State

@onready var dash_draugr : DashDraugr = $"../.."
@onready var patrol : State = $"../DashDraugrStatePatrol"
@onready var melee_attack : State = $"../DashDraugrStateAttack"
@onready var chase : State = $"../DashDraugrStateChase"
...
func state_process(delta : float) -> State:
	...
	if !cooldown:
		if dash_draugr.global_position.distance_to(player.global_position) >= raycast_component.raycast_length:
			return patrol
		elif dash_draugr.global_position.distance_to(player.global_position) < 100:
			dash_draugr.direction = (player.global_position - dash_draugr.global_position).normalized()
...
func start_cooldown(time: float = 2.0) -> void:
	if not cooldown:
		cooldown = true
		cooldown_timer = time
