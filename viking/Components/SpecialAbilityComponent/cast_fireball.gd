...
var upgrade_level = 3 #From Will to Josh, this will be what will determine the special upgrade. Just connect a signal and a _on_signal_name_recieved to change it.
...
func cast_ability() -> void:
	if Global.patron_god == 1 and upgrade_level == 1: #base lightning
	...
	elif Global.patron_god == 1 and upgrade_level == 2: #three lightning shot.
		var new_fireball = fireball.instantiate()
